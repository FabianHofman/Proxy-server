...
namespace Proxy_server.Proxy
{
    public class Server
    {
        ...
        public void Start()
        {
            if(ValidateServerPreferences(Port, BufferSize))
            {
                try
                {
        ...
        private async Task HandleClient(TcpClient incomingClient)
        {
            ...
            using (NetworkStream incomingClientStream = incomingClient.GetStream())
            {
                ...
                if (httpRequest != null)
                {
                    if (HideUserAgentEnabled)
                    {
                        httpRequest.HideUserAgent();
                    }
                    ...
                    if (LogRequest) AddToLog($"\r\n----------Request Received----------\r\n{httpRequest.ToString}\r\n");
                        ...
                        if (httpRequest.IsCacheable() && _cacheHandler.IsStoredInCache(httpRequest.FirstLine))
                        {
                            ...
                            if (cacheItem.IsValid(CacheTimeOutInSeconds))
                            {
                                ...
                                AddToLog("Found in cache and using it");
                        ...
                        if (httpResponse == null)
                        {
                            ...
                            if (httpRequest.IsCacheable())
                            {
                                _cacheHandler.TryStoreInCache(httpRequest, httpResponse);
                            }
                        }
                    if (httpResponse != null && LogResponse)
                    {
        ...
        private bool ValidateServerPreferences(int portNumber, int bufferSize)
        {
